...
void XMatmul_optimized_Set_AB(XMatmul_optimized *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XMatmul_optimized_WriteReg(InstancePtr->Ctrl_BaseAddress, XMATMUL_OPTIMIZED_CTRL_ADDR_AB_DATA, (u32)(Data));
    XMatmul_optimized_WriteReg(InstancePtr->Ctrl_BaseAddress, XMATMUL_OPTIMIZED_CTRL_ADDR_AB_DATA + 4, (u32)(Data >> 32));
}

u64 XMatmul_optimized_Get_AB(XMatmul_optimized *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XMatmul_optimized_ReadReg(InstancePtr->Ctrl_BaseAddress, XMATMUL_OPTIMIZED_CTRL_ADDR_AB_DATA);
    Data += (u64)XMatmul_optimized_ReadReg(InstancePtr->Ctrl_BaseAddress, XMATMUL_OPTIMIZED_CTRL_ADDR_AB_DATA + 4) << 32;
    return Data;
}
