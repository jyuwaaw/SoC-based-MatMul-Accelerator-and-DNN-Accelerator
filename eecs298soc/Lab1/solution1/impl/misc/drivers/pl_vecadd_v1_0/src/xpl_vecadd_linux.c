...
typedef struct {
    ...
    XPl_vecadd_uio_map maps[ MAX_UIO_MAPS ];
} XPl_vecadd_uio_info;

/***************** Variable Definitions **************************************/
static XPl_vecadd_uio_info uio_info;
...
static int uio_info_read_name(XPl_vecadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPl_vecadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPl_vecadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPl_vecadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPl_vecadd_Initialize(XPl_vecadd *InstancePtr, const char* InstanceName) {
	XPl_vecadd_uio_info *InfoPtr = &uio_info;
