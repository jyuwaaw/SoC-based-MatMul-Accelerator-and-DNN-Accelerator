...
entity matmul_plain_ctrl_s_axi is
generic (
...
end entity matmul_plain_ctrl_s_axi;
...
architecture behave of matmul_plain_ctrl_s_axi is
    ...
    constant ADDR_AB_DATA_0 : INTEGER := 16#28#;
    constant ADDR_AB_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_AB_CTRL   : INTEGER := 16#30#;
    ...
    signal int_AB              : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_AB_DATA_0 =>
                        rdata_data <= RESIZE(int_AB(31 downto 0), 32);
                    when ADDR_AB_DATA_1 =>
                        rdata_data <= RESIZE(int_AB(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_AB(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_AB_DATA_0) then
                    int_AB(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_AB(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_AB(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_AB_DATA_1) then
                    int_AB(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_AB(63 downto 32));
                end if;
            end if;
        end if;
    end process;
